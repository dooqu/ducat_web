...
namespace api
{
    ...
    public class WeixinServiceProvider
    {
        ...
        public WechatSessionInfo GetSessionInfoByJscode(string code)
        {
            ...
            WechatSessionInfo session_info = JsonConvert.DeserializeObject<Modal.WechatSessionInfo>(response_content);
            return session_info;
        }

        public string CreateWechatOrder(WechatOrderInfo order)
        {
            string service_url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
            System.Diagnostics.Trace.WriteLine("发送:" + order.ToXmlString());
            return GetResponseStringByPost(service_url, order.ToXmlString());
        }
    }
}